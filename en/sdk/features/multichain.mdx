...
<Note>
  ...
  can [set these on a per-processor basis](cloud/reference/manifest/#processor)
...
If you plan to use [`sqd run`](squid-cli/run) for local runs or deploy to [SQD Cloud](cloud), list your processors in the `deploy.processor` section of your [deployment manifest](cloud/reference/manifest/#processor):
