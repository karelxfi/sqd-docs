...
* The argument of [`addInstruction()`](/en/sdk/squid-sdk/solana/sdk/solana-batch/instructions) is a set of filters that tells the processor to retrieve all data on the swap instruction of the Whirlpool program with discriminator matching the hash of the `<namespace>:<instruction>` of the swap instruction.
* The argument of [`setFields()`](/en/sdk/squid-sdk/solana/sdk/solana-batch/field-selection) specifies the exact fields we need for every data item type.

See [`SolanaDataSource` reference](/en/sdk/squid-sdk/solana/sdk/solana-batch) for more options.
...
The other part the squid processor (the ingester process of the indexer) is the callback function used to process batches of the filtered data, the [batch handler](/en/sdk/squid-sdk/features/batch-processing). In Solana Squid SDK it is typically defined within a `run()` call, like this:
```typescript
import \{run\} from '@subsquid/batch-processor'
...
* `database` is a [`Database` implementation](/en/sdk/squid-sdk/features/persisting-data) specific to the target data sink. We want to store the data in a PostgreSQL database and present with a GraphQL API, so we provide a [`TypeormDatabase`](/en/sdk/squid-sdk/features/persisting-data/typeorm) object here.
* `ctx` is a [batch context](/en/sdk/squid-sdk/features/batch-processing) object that exposes a batch of data (at `ctx.blocks`) and any data persistence facilities derived from `db` (at `ctx.store`). See [Block data for Solana](/en/sdk/squid-sdk/solana/sdk/solana-batch/context-interfaces) for details on how the data batches are presented.
