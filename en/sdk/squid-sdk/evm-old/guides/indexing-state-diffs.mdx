...
<Tip>
State diffs for historical blocks are [currently available](subsquid-network/reference/networks/#evm--ethereum-compatible) from [SQD Network](subsquid-network) on the same basis as all other data stored there: for free. If you deploy a squid that indexes traces [in real-time](sdk/resources/unfinalized-blocks) to SQD Cloud and use our [RPC addon](/en/cloud/resources/rpc-proxy), the necessary `trace_` or `debug_` RPC calls made will be counted alongside all other calls and [the price](/en/cloud/pricing/#rpc-requests) will be computed for the total count. There are no surcharges for traces or state diffs.
</Tip>
