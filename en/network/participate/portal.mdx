...
SQD Network datasets are partitioned by block number. Dataset chunks are randomly distributed among [worker nodes](subsquid-network/participate/worker).
...
num_chunks_in_range = range_length _ (eth_chunks / eth_height) _ (chain_txs_per_block / eth_txs_per_block)
