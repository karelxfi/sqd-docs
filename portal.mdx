...
## Slide 12 — Live demo plan (happy path)

**Goal:** Ingest ERC‑20 transfers for a token into ClickHouse in <60s.

1. Start standalone Portal sidecar against `anvil` or remote L2.
2. Create `erc20_transfers` table.
3. Run `portal‑ingest.ts` (shown earlier).
4. `SELECT count()` and a sample query by address.
5. Show resume from cursor after intentional interruption.
6. (If venue bandwidth allows) briefly point a second terminal at a **high‑TPS chain** range to show constant‑memory streaming.

Speaker notes:

- Keep env deterministic (seeded Anvil or fixed L2 range). Have a pre‑cut high‑TPS clip if Wi‑Fi is flaky.

---

## Slide 13 — Live demo: reorg handling (optional)

1. Flip `finalityLag=0` and stream near head.
2. Trigger a synthetic reorg (Anvil/Hardhat for EVM; fork reset).
3. Observe `reorg` event; Portal issues compensation sequence.
4. Verify CH table contains corrected state (idempotent upserts by `(block, tx, logIndex)`).

Speaker notes:

- If time squeezed, keep this as a recorded snippet.

---

## Slide 14 — Fallback demo (offline‑safe)

- Pre‑cut `demo.tar.gz` with:

  - small ClickHouse data dir + `demo.db` (or DuckDB)
  - `portal‑ingest.ts` and `.env.example`
  - `dataset.ndjson` (10k logs)

- If Wi‑Fi is flaky: run ingestion from local NDJSON stream (same SDK call with `file://`).

Speaker notes:

- Never depend on venue Wi‑Fi for the wow‑moment.
...
- **Prod**: SQD Network endpoints → VPC CH/S3; managed by Subsquid Cloud
...
- Talk TCO qualitatively; avoid unverifiable benchmarks on stage.

---

## Slide 17 — How to try it

- `docker run subsquid/portal-sidecar:latest`
- `npx @subsquid/portal-sdk init`
- SQD Network endpoints: `https://portal.sqd.network/{chain}/...` (examples)
- Managed endpoints: Subsquid Cloud

Speaker notes:

- Give a single command for each persona.

---

## Slide 18 — Call to action (for hackers)

- Ship a data‑heavy hack without writing an indexer
- Targets: ClickHouse, Postgres, S3, BigQuery (via FDW)
- Bounties for the best Portal pipelines this weekend

Speaker notes:

- Make it immediately actionable at the hackathon.

---

## Slide 19 — Roadmap & contributions

- Traces across more clients; Merkle proofs on wire; Solana accounts diffs
- More SDK adapters (Kafka, Parquet/S3, Delta Lake)
- PRs welcome: sidecar, SDK, network adapters

Speaker notes:

- Invite contributors at the right abstraction layer.

---

## Slide 20 — Thank you & links

- **EVM Portal docs**: [https://docs1.sqd.ai/en/home](https://docs1.sqd.ai/en/home)
- **Solana Portal (Soldexer) docs**: [https://docs.soldexer.dev/get-started/introduction](https://docs.soldexer.dev/get-started/introduction)
- **Pipes SDK (data pipelines)**: [https://github.com/subsquid-labs/pipes-sdk-docs](https://github.com/subsquid-labs/pipes-sdk-docs)
- **Sidecar / SDK repos**: see docs above for direct GitHub links

Speaker notes:

- Keep QR codes to these three docs ready (EVM, Soldexer, Pipes SDK).

---

# Live demo script (step‑by‑step)

### 0. Prep (do this before going on stage)

- `docker pull subsquid/portal-sidecar:latest`
- Clone `portal-demo` repo with scripts:

  - `portal-ingest.ts` (ClickHouse) and `env.example`
  - `schema.sql` for CH

- Start local ClickHouse: `docker compose up -d` (or use DuckDB offline fallback)
- Verify sidecar connects to your Anvil/Geth or SQD Network endpoint.

### 1. Start sidecar (visible on stage)

```bash
PORTAL_UPSTREAM_RPC=$RPC \
PORTAL_NETWORK=evm:mainnet \
docker run --rm -p 8080:8080 subsquid/portal-sidecar:latest
```

- Show logs printing: listening on :8080; upstream healthy.

### 2. Create table

```bash
clickhouse-client --query "CREATE TABLE IF NOT EXISTS erc20_transfers (block UInt32, tx String, logIndex UInt16, from String, to String, value String) ENGINE=MergeTree ORDER BY (block, tx, logIndex)"
```

### 3. Run ingestion

```bash
bun run portal-ingest.ts
```

- Terminal prints rows/sec and current block.

### 4. Show data

```bash
clickhouse-client --query "SELECT count() FROM erc20_transfers"
clickhouse-client --query "SELECT * FROM erc20_transfers ORDER BY block DESC LIMIT 5"
```

### 5. Interrupt & resume

- `Ctrl+C` the ingest script.
- Re‑run with `--cursor $LAST_CURSOR` or rely on SDK’s persisted cursor file.

### 6. (Optional) Reorg demo

- With Anvil: `anvil --block-time 1 --hardfork cancun` and trigger a short fork.
- Observe `reorg` event in the stream and table correction.
...
- [ ] Laptop power + HDMI/USB‑C dongle
- [ ] Local ClickHouse or DuckDB fallback tested
- [ ] Pre‑pulled Docker images (sidecar, CH)
- [ ] `.env` with stable RPC or local Anvil
- [ ] Demo dataset `.ndjson` cached
- [ ] QR codes to repos/docs
...
- **EVM Portal docs**: [https://docs1.sqd.ai/en/home](https://docs1.sqd.ai/en/home)
- **Solana Portal (Soldexer) docs**: [https://docs.soldexer.dev/get-started/introduction](https://docs.soldexer.dev/get-started/introduction)
- **Pipes SDK docs**: [https://github.com/subsquid-labs/pipes-sdk-docs](https://github.com/subsquid-labs/pipes-sdk-docs)
