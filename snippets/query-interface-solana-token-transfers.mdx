export const QueryInterfaceSolanaTokenTransfers = () => {
  const useCasesConfig = [
    {
      id: "usdc-token-transfers",
      name: "Track USDC token transfers",
  ...
  const [useCase, setUseCase] = useState("usdc-token-transfers");
