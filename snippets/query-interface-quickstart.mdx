export const QueryInterfaceQuickstart = () => {
  const useCasesConfig = [
    {
      id: "ethereum-blocks",
      name: "Query Ethereum blocks",
  ...
  const [useCase, setUseCase] = useState("ethereum-blocks");
...
<div className="not-prose w-full max-w-7xl mx-auto px-4 sm:px-6 my-6 sm:my-8">
      ...
      <div className="bg-white dark:bg-gray-900 rounded-xl p-4 sm:p-6 border border-gray-200 dark:border-gray-800 shadow-sm">
          ...
          <div className="flex flex-wrap items-center gap-3 flex-1 w-full">
            <div className="relative flex-shrink-0">
              <select
                ...
                className="appearance-none bg-gray-50 dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded-xl px-4 py-2.5 pr-10 text-sm font-medium text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400 cursor-pointer transition-all"
              >
            ...
            <div className="relative flex-shrink-0 flex-1 min-w-[200px]">
              <select
                ...
                className="appearance-none w-full bg-gray-50 dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded-xl px-4 py-2.5 pr-10 text-sm font-medium text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400 cursor-pointer transition-all"
              >
            ...
            <button
              ...
              type="button"
              disabled={loading}
              className="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 dark:bg-blue-500 dark:hover:bg-blue-600 disabled:bg-gray-400 disabled:cursor-not-allowed text-white font-semibold px-6 py-2.5 rounded-xl transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 dark:focus:ring-offset-gray-900 whitespace-nowrap shadow-sm hover:shadow-md"
            >
              {loading ? (
                <>
        ...
        <div className="mt-4 bg-gray-50 dark:bg-gray-950 rounded-xl border border-gray-200 dark:border-gray-800 overflow-hidden h-[450px] flex flex-col">
              ...
              <div key={`code-${useCase}`} style={{ position: 'relative' }}>
                <pre style={{
                  ...
                  <code key={`curl-code-${useCase}`} style={{ counterReset: 'line' }}>
                    {generateCurlCommand(useCasesConfig.find(uc => uc.id === useCase)).split('\n').map((line, i) => (
                      <div key={`curl-line-${i}`} style={{
                        ...
                        <span style={{ whiteSpace: 'pre-wrap', wordBreak: 'break-word', color: '#24292f' }}>
                          {line.includes('curl') && <span style={{ color: '#8250df' }}>{line}</span>}
                          {!line.includes('curl') && line.match(/^\s*-[XHd]/) && (
                            <>
                              <span style={{ color: '#8250df' }}>{line.match(/^\s*-[XHd]/)[0]}</span>
                      ...
                      <div key={`response-line-${i}`} style={{
                        ...
                        <span style={{ whiteSpace: 'pre-wrap', wordBreak: 'break-word', color: '#24292f' }}>
                          {line.match(/"[^"]+":/) ? (
                            <>
                              {line.split(/("[^"]+":)/).map((part, j) =>
                                part.match(/^"[^"]+":$/) ? <span key={`resp-part-${i}-${j}`} style={{ color: '#0550ae' }}>{part}</span> :
