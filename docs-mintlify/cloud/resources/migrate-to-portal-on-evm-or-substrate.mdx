...
<Info>
  ...
  limits. If you see an HTTP 400 error with a message like this: ``` Couldn't
  parse query: query contains X item requests, but only 50 is allowed ``` where
  `X` is some number above 50, or any other HTTP 400 response, please let us
  know.
</Info>
